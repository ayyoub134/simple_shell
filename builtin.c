 ...
 */
int custom_exit(shell_info_t *shell_info)
{
	int exit_check;

	if (shell_info->argv[1])  /* If there is an exit argument */
	{
		exit_check = custom_atoi(shell_info->argv[1]);
		if (exit_check == -1)
		{
			shell_info->status = 2;
		...
		shell_info->err_num = custom_atoi(shell_info->argv[1]);
		return (-2);
	}
 ...
 *  Return: Always 0
 */
int custom_cd(shell_info_t *shell_info)
{
 ...
 *  Return: Always 0
 */
int custom_help(shell_info_t *shell_info)
{
	char **arg_array;
